...
export const list = catchAsync(async (req, res) => {
  ...
  const result = await getOwnerNotifications(req.ownerId, pagination);
...
export const markRead = catchAsync(async (req, res) => {
  const notification = await markAsRead(req.params.id, req.ownerId);
...
export const markAllRead = catchAsync(async (req, res) => {
  await markAllAsRead(req.ownerId);
  return ApiResponse.success(res, 200, 'All notifications marked as read');
});

export const remove = catchAsync(async (req, res) => {
  await deleteNotification(req.params.id, req.ownerId);
