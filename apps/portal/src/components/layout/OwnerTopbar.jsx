...
export default function OwnerTopbar({ onMenuToggle }) {
  ...
  return (
    <>
      {/* Mobile topbar */}
      <header className="md:hidden flex justify-between items-center px-6 h-16 w-full sticky top-0 z-30 bg-surface text-dark border-b border-border">
      ...
      {/* Desktop topbar */}
      <header className="hidden md:flex justify-between items-center px-8 h-20 w-full sticky top-0 z-30 bg-surface/80 backdrop-blur-md border-b border-border">
        <div className="flex-1">
          <div className="relative max-w-sm group">
            ...
            <input
              className="w-full bg-surface-dim border border-transparent rounded-btn pl-12 pr-4 py-3 text-xs font-bold text-dark focus:bg-surface focus:border-border transition-all outline-none"
        ...
        <div className="flex items-center gap-6 relative">
          <Link to="/notifications" className="text-muted hover:text-dark transition-colors p-2 rounded-btn hover:bg-surface-dim active:scale-95 relative">
