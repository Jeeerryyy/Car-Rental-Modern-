...
export default function AddCar() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createCar({ ...form, pricePerDay: Number(form.pricePerDay), year: Number(form.year), seats: Number(form.seats), images });
  ...
  return (
    <div className="p-6 lg:p-12 max-w-[1000px] mx-auto w-full pb-24 md:pb-6">
      <div className="mb-8">
        <h2 className="font-headline-xl text-headline-xl text-primary mb-2">Add New Car</h2>
        <p className="text-on-surface-variant">Fill in the details to add a vehicle to your fleet</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" value={form.make} onChange={set('make')} placeholder="e.g. Maruti" className={inputClass('make')} />
              {errors.make && <p className="text-red-500 text-xs mt-1">{errors.make}</p>}
            </div>
            <div>
              <label className="block text-sm font-semibold text-dark mb-2">Model *</label>
              <input type="text" value={form.model} onChange={set('model')} placeholder="e.g. Swift Dzire" className={inputClass('model')} />
            ...
            <div>
              ...
              <input type="number" value={form.seats} onChange={set('seats')} min="2" max="12" className={inputClass('seats')} />
            </div>
            <div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-dark mb-2">Transmission</label>
              <select value={form.transmission} onChange={set('transmission')} className={inputClass('transmission')}>
                {TRANSMISSIONS.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-semibold text-dark mb-2">Drive Option</label>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="number" value={form.pricePerDay} onChange={set('pricePerDay')} min="0" placeholder="1500" className={inputClass('pricePerDay')} />
          ...
          <div>
            ...
            <textarea value={form.description} onChange={set('description')} rows="4"
              placeholder="Describe the vehicle features, condition, and any extras…"
        ...
        <div className="flex justify-end gap-4">
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? 'Adding...' : 'Add Car to Fleet'}
          </button>
        </div>
      </form>
    </div>
