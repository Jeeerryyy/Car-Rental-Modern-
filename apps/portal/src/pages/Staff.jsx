  ...
  const handleToggleStatus = async (id) => {
    try {
      ...
      setStaffList(prev => prev.map(s => 
        s._id === id ? { ...s, status: s.status === 'active' ? 'inactive' : 'active' } : s
      ));
            ...
            <div key={staff._id} className="bg-surface-container-lowest border border-outline-variant rounded-3xl p-6 hover:shadow-xl transition-all group relative overflow-hidden">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-xl font-bold text-on-surface tracking-tight">{staff.name}</h4>
                    <p className="text-sm text-on-surface-variant font-medium">{staff.email}</p>
                  </div>
                </div>
                <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${staff.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {staff.status}
                </div>
              </div>
